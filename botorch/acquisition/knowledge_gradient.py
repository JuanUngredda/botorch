...
class MyCustomSampler(SobolQMCNormalSampler):
    def __init__(self, base_samples=None, num_samples=1, seed=None, **kwargs):
        super(MyCustomSampler, self).__init__(num_samples=num_samples, seed=seed, **kwargs)
        self.base_samples = None

    def sample(self, X):
        # Implement your custom sampling logic here
        # X is the input space for the acquisition function
        # Return samples based on your custom logic
        pass
...
class MCKnowledgeGradient(DiscreteKnowledgeGradient):
    ...
    def compute_mc_kg(
        ...
        for fantasy_idx in range(self.num_fantasies):
            ...
            sampler.base_samples = torch.Tensor([[zvalues[fantasy_idx]]])
            ...
            with torch.enable_grad():
                ...
                # print("xnew: " + str(xnew))
                # print(" base samples: " + str(sampler.base_samples))
                # plt.scatter(Xplot, mu_val_initial_conditions_raw.numpy().squeeze())
                # plt.scatter(xval, value.detach().numpy(), color="red")
                # plt.show()
                ...
                with settings.propagate_grads(True):
                    ...
                    if self.current_value is not None:
                        x_current_optimiser_val = value_function(X=self.current_optimiser)
                        if x_current_optimiser_val > x_top_val:
                            x_top_val = x_current_optimiser_val

                if self.current_value is not None:
                    x_top_val = x_top_val - x_current_optimiser_val
...
class HybridOneShotKnowledgeGradient(qKnowledgeGradient):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        for x_i, xnew in enumerate(X_actual):
            X_discretisation = X_fantasies[:, x_i, ...].squeeze(1)
