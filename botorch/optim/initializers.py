...
def compute_x_star( model: Model,
                    bounds: Tensor,
                    xnew: Tensor,
                    zvalues: Tensor,
                    num_restarts: int,
                    raw_samples: int) -> Tensor:
    """
    Args:
    ...
    num_fantasies = len(zvalues.squeeze())
    xstar_inner_optimisation = torch.zeros((num_fantasies, xnew.shape[1]))
    ...
    for fantasy_idx in range(num_fantasies):
    ...
    return xstar_inner_optimisation

def gen_hybrid_one_shot_kg_initial_conditions(
    acq_function: HybridOneShotKnowledgeGradient,
    bounds: Tensor,
    q: int,
    num_restarts: int,
    raw_samples: int,
    ...
    X_actual, X_fantasies = _split_fantasy_points(X=ics, n_f = num_fantasies)

    optimised_ics = torch.zeros(ics.shape)
    for idx, xnew in enumerate(X_actual):
        xstar = compute_x_star(model=acq_function.model,
                       xnew=xnew,
                       bounds=bounds,
                       num_restarts=1,
                       raw_samples=100,
                       zvalues=acq_function.construct_z_vals(nz=num_fantasies))
        idx_ics = torch.cat([xnew, xstar])
        optimised_ics[idx, ...] = idx_ics

    return optimised_ics
